...
%   $Revision: 1.1.6.9 $  $Date: 2005/12/01 03:11:11 $
%==============================================================================
% Set default values.
Sheet1 = 1;
...
try
    ...
    try
        ...
        try
            ...
            %Open file
%             ExcelWorkbook = Excel.workbooks.Open(file);
            named_ranges=ExcelWorkbook.Names;
            n_record=named_ranges.count;
            for counter=1:n_record
                record_det=named_ranges.Item(counter);
                record_name=record_det.Name;
                if(strcmp(record_name,range)) % TODO: not sure if they are case sensitive
                    range=get(record_det,'RefersToLocal');
                    break
                end
            end
...
%             ExcelWorkbook.Close
    ...
    try % select region.
        ...
        err = lasterror;
        err.identifier = 'MATLAB:xlswrite:SelectDataRange';
        err.message = sprintf('Excel returned: %s.', err.message); 
        error(err);
    end
